...
function exibirMensagemInicial() {
    exibirTexto('h1', 'Ajude Mário Adivinhão a descobrir o número secreto!');
...
function gerarNumeroAleatorio() {
    ...
    let quantidaDeNumerosNaLista = listaDeNumerosSecretosSorteados.length;

    if (quantidaDeNumerosNaLista == numeroLimite) {
        listaDeNumerosSecretosSorteados = [];
    }
    if (listaDeNumerosSecretosSorteados.includes(numeroGerado)) {
    } else {
        listaDeNumerosSecretosSorteados.push(numeroGerado);
        console.log(listaDeNumerosSecretosSorteados);
        return numeroGerado;
    }
}
